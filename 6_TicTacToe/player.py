...
class GeniusComputerPlayer(Player):
    ...
    def get_move(self, game):
        if len(game.available_moves()) == 9:
            # square = self.minimax(game, self.letter)['position'] fixme 总是选择左上角，并有错误策略
            square = random.choice(game.available_moves())  # randomly choose one fixme 4 for must win?
        else:
    ...
    def minimax(self, state, player):
        ...
        for possible_move in state.available_moves():
            ...
            else:  # best minimize the other player
                if sim_score['score'] < best['score']:
                    best = sim_score  # replase best
