...
class Board:
    def __init__(self, dim_size, num_bombs):
        # let's keep track of these of these parameters. they'll be helpful later
    ...
    def make_new_board(self):
        ...
        while bombs_planted < self.num_bombs:
            ...
            col = loc % self.dim_size  # we want the remainder to tell what index in that row to look at
    ...
    def get_num_neighboring_bombs(self, row, col):
        ...
        for r in range(max(0, row - 1), min(self.dim_size - 1, row + 1) + 1):  # 注意for的性质
    ...
    def dig(self, row, col):
        ...
        for r in range(max(0, row - 1), min(self.dim_size - 1, row + 1) + 1):
            for c in range(max(0, col - 1), min(self.dim_size - 1, col + 1) + 1):
                if (r, c) in self.dug:
                    continue  # don't dig where you've dug
                self.dig(r, c)  # 递归
    ...
    def __str__(self):
        ...
        visible_board = [[None for _ in range(self.dim_size)] for _ in range(self.dim_size)]  # 用户看到的board
...
def play(dim_size=10, num_bombs=10):
    ...
    while len(board.dug) < board.dim_size ** 2 - num_bombs:
        print(board)
        # 0,0 or 0, 0 or 0,     0  通过正则表达式忽略“,”后面所有空格并拆分，以使前面输入均合法
        user_input = re.split(',(\\s)*', input("Where would you want to dig? Input as row,col: "))  # '0, 3'
        row, col = int (user_input[0]), int(user_input[-1])  # 提取第一个字符和最后一个字符
